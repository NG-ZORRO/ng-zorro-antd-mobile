...
const template = String(fs.readFileSync(path.resolve(__dirname, '../template/code.template.html')));
const capitalizeFirstLetter = require('./capitalize-first-letter');

module.exports = function generateCode(component, key, title, doc, iframe) {
  let output = template;
  output = output.replace(/{{title}}/g, title);
  output = output.replace(/{{component}}/g, component);
  const componentName = `Demo${camelCase(capitalizeFirstLetter(component))}${camelCase(capitalizeFirstLetter(key))}Component`;
